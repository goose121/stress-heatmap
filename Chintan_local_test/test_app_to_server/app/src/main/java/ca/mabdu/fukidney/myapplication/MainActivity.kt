            ...
            Scaffold(
                ...
                Column(
                    ...
                    Button(
                        ...
                        Text(
                            when(networkState) {
                                NetworkState.IDLE -> "Submit with 'no location atm!'" //change
                                NetworkState.BUSY -> "Sending... a beating"
                            }
                        )
                    }
                }
    ...
    private fun sendNetworkData(
        ...
        scope.launch(Dispatchers.IO) {
            ...
            if (location == null) {
                ...
                // NEW STUFF! - sends text if location fails
                try{
                    val result = NetworkHandler.sendTextToFastAPI(data)
                    runOnUiThread {
                        Toast.makeText(
                            ctx,
                            "Text sent (no location): $result",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }catch (e: Exception) {
            ...
            // WOW SUPER NEW
                val textWithLocation = "$data [Lat: ${location.latitude}, Lon: ${location.longitude}]"
                val result = NetworkHandler.sendTextToFastAPI(textWithLocation)
                // also new
                runOnUiThread {
                    Toast.makeText(
                        ctx,
                        "Sent to FastAPI: $result",
                        Toast.LENGTH_LONG
                    ).show()
                }
